...
void LIA::XmlLoader::parseNode(XmlData& xmlData, rapidxml::xml_node<> * root, std::string parentName) {
    for (rapidxml::xml_node<> *child = root->first_node(); child; child = child->next_sibling()) {
        ...
        if (type == rapidxml::node_type::node_data) {
            LIA_trace(std::vformat("Adding value for '{}'", std::make_format_args(child->name())));
            ...
            LIA_trace(std::vformat("{}.type = {}", std::make_format_args(child->name(), cType)));
        }
        XmlNode xmlNode;
...
void LIA::XmlLoader::printNode(rapidxml::xml_node<> *node) {
    ...
    LIA_debug(std::vformat("node.name = {}; node.attr = [", std::make_format_args(node->name())));
    for (rapidxml::xml_attribute<> *attr = node->first_attribute(); attr; attr = attr->next_attribute())
    {
        LIA_debug(std::vformat("{} = {}", std::make_format_args(attr->name(), attr->value())));
    }
    LIA_debug("]");
    if (type == rapidxml::node_type::node_data) {
        LIA_debug(std::vformat("node.value = {}", std::make_format_args(node->value())));
    }
