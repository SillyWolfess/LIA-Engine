...
bool LIA::TextureManager::init() {
    if (!registerTexture("empty", "./data/emptyTextures/", "Shader_NoneWhite.png", TextureType::RGBA)) {
        LIA_error("Failed to register empty texture");
        return false;
    }
    if (!registerTexture("emptyBump", "./data/emptyTextures/", "Shader_NoneNormal.png", TextureType::ANY)) {
        LIA_error("Failed to register emptyBump texture");
        return false;
    }
    if (!registerTexture("emptyEm", "./data/emptyTextures/", "Shader_NoneBlack.png", TextureType::HDR)) {
