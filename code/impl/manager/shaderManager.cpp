...
bool LIA::ShaderManager::validate(ShaderProgram& shader) {
    ...
    if (shader.getProgram() != internalId) {
        LIA_fatal_f("Program Ids do not match. Program name = {}. Program Id = {}. Stored Id = {}.", shader.getName(), shader.getProgram(), internalId);
        return false;
    }
    return true;
}
...
bool LIA::ShaderManager::init() {
    ...
    for (ShaderProgram &shader : _shaders) {
        if (!shader.load()) {
            LIA_fatal_f("{} failed to load", shader.getName());
            return false;
        }
        if (!validate(shader)) {
            LIA_fatal_f("{} validation failed", shader.getName());
            return false;
        }
    }
